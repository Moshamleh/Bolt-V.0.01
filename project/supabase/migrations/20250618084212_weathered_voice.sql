/*
  # Club Messages and Club Founder Badge

  1. New Tables
    - Ensures club_messages table exists for club communication
  2. Security
    - Enables RLS on club_messages table
    - Adds policies for club members to read and send messages
  3. Data
    - Adds Club Founder badge if it doesn't exist
*/
...
-- Create policy for club members to read messages (with IF NOT EXISTS)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polname = 'Club members can read messages' 
    AND polrelid = 'club_messages'::regclass
  ) THEN
...
-- Create policy for club members to send messages (with IF NOT EXISTS)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
