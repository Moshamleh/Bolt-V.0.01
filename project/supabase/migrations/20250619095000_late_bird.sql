/*
  # Club messaging system and badges
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'club_messages' AND policyname = 'Club members can read messages'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'club_messages' AND policyname = 'Club members can send messages'
  ) THEN
