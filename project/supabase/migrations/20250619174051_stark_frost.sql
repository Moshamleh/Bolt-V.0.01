/*
  # Consolidated Idempotent Migration File

  This migration consolidates and fixes all the problematic migrations that were causing recurring errors.
  It's designed to be fully idempotent - it can be run multiple times without causing errors.
  ...
  2. Policies
    - Properly drops and recreates all policies
    - Uses correct syntax for policy existence checks

  3. Functions and Triggers
    - Creates or replaces all necessary functions
    - Properly handles trigger creation

  4. Data
    - Inserts initial data with ON CONFLICT clauses to prevent duplicates
*/
...
DROP POLICY IF EXISTS "Admins can read reports" ON reported_parts;
...
DROP POLICY IF EXISTS "Users can report parts" ON reported_parts;
...
DROP POLICY IF EXISTS "Anyone can read seller reviews" ON seller_reviews;
...
DROP POLICY IF EXISTS "Users can create reviews as buyers" ON seller_reviews;
...
DROP POLICY IF EXISTS "Users can update their own reviews" ON seller_reviews;
...
DROP POLICY IF EXISTS "Users can delete their own reviews" ON seller_reviews;
...
DROP POLICY IF EXISTS "Admins can manage all reviews" ON seller_reviews;
...
DROP POLICY IF EXISTS "Users can read their own challenge progress" ON user_challenges;
...
DROP POLICY IF EXISTS "Users can read own boost orders" ON boost_orders;
...
DROP POLICY IF EXISTS "Users can create boost orders" ON boost_orders;
...
DROP POLICY IF EXISTS "Users can update own boost orders" ON boost_orders;
...
DROP POLICY IF EXISTS "Service role can update boost orders" ON boost_orders;
...
DROP POLICY IF EXISTS "Service role can insert boost orders" ON boost_orders;
...
DROP POLICY IF EXISTS "Service role can read boost orders" ON boost_orders;
...
-- Add unique constraint to challenges name column if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    ...
    ALTER TABLE challenges ADD CONSTRAINT unique_challenge_name UNIQUE (name);
  END IF;
END $$;
