...
import {
  ...
  Filter,
  X,
  Video, Calendar, Clock, DollarSign, Navigation, Loader2
,
  Video,
  Calendar,
  Clock,
...
const MechanicSupportPage: React.FC = () => {
  ...
  const [selectedSpecialties, setSelectedSpecialties, nearbyOnly, userLocation] = useState<string[]>([]);
  ...
  const [distancePricing, setDistancePricing] = useState<DistancePricing | null>(null);
  ...
  const handleStartVideoCall = async (mechanic: Mechanic, callType: 'voice' | 'video') => {
    try {
  ...
  const handleCallEnd = (duration: number, totalCost: number) => {
    ...
    toast.success(`Call ended. Duration: ${Math.floor(duration / 60)}:${(duration % 60).toString().padStart(2, '0')}`);
  };
  ...
  const calculateDistance = (mechanic: Mechanic) => {
    ...
    const a = 
      ...
      Math.cos(userLocation.lat * Math.PI / 180) * Math.cos(mechanic.latitude * Math.PI / 180) * 
              ...
              <div className="mt-2 space-y-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                {mechanic.average_rating && (
                  <div className="flex items-center gap-2">
                    ...
                    <span>{mechanic.average_rating.toFixed(1)} ({mechanic.total_reviews} reviews)</span>
                  </div>
                )}
              </div>
                ...
                {mechanic.is_available_for_calls && (
                  <>
                    ...
                    >
                      ...
                      {startingCall === mechanic.id ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Call'}
                    </button>
                    ...
                    >
                      ...
                      {startingCall === mechanic.id ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Video'}
                    </button>
                  </>
                )}
              </div>
